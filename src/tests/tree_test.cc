...
using tree = s21::tree<const int, int>;
using pair = std::pair<const int, int>;
using str = std::string;
using init_list = std::initializer_list<int>;
using std::cout;

TEST(tree, nullNodeInsertLimit) {
  std::initializer_list<pair> items = {{}};
  // int res[] = {1, 2, 3, 4};
  tree t{items};
  // cout<<t.size();
  // cout<<t.structure();
  int i = 0;
  for (auto it1 = t.begin(); it1 != t.end(); i++, it1++) {
    EXPECT_TRUE((*(static_cast<tree::const_iterator>(it1))).second ==
                pair{}.second);
    //(*(static_cast<tree::const_iterator>(it1))).second = 5;
    // cout<<(*it1).first<<' '<<(*it1).second;
  }
}

TEST(tree, typeCastToConst) {
  ...
  int i = 0;
  for (auto it1 = t.begin(); it1 != t.end(); i++, it1++) {
    EXPECT_TRUE((*(static_cast<tree::const_iterator>(it1))).second == res[i]);
    //(*(static_cast<tree::const_iterator>(it1))).second = 5;
    (*it1).second = 5;
  }
}
...
TEST(tree, copyConstructor) {
  ...
  tree t2{t1};
  EXPECT_TRUE(t2.size() == t1.size());
  for (auto it1 = t1.begin(), it2 = t2.begin();
...
TEST(tree, moveConstructor) {
  ...
  int i = 0;
  for (auto it2 = t2.begin(); it2 != t2.end(); i++) {
    EXPECT_TRUE((*it2).first == res[i]);
    ++it2;
  }
  // t1.structure();
  // cout<<t2.structure();
